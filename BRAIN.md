...
- 2026-07-03: Career Break image refresh completed. Generated a square travel-themed beach lounger/cocktail illustration via the built-in image generator, copied it into `public/career-break-beach.png` while leaving the original generated file under `/Users/bd0t/.codex/generated_images/019f27b2-cf0c-7100-89c7-98e8d1d93e61/`, and updated the `careerBreak` timeline entry to use `/career-break-beach.png` instead of `/BY-logo.png`. Added a Resume.exe regression that checks the career-break image path, confirms the public PNG exists, verifies it is non-empty, and validates the PNG signature. Verification passed with focused `node --test src/scripts/resume-app.test.mjs`, full `node --test src/scripts/*.test.mjs` at 149/149, `env ASTRO_TELEMETRY_DISABLED=1 npm run build` with 6 pages, built-output checks showing `dist/career-break-beach.png` at 2,536,827 bytes, and `cmp -s public/career-break-beach.png dist/career-break-beach.png`.
- 2026-07-03: Career Break copy refresh completed. Replaced the initial placeholder career-break prose with the user-provided travel/recharge/freedom wording in `src/content/timeline.json`, kept it as the current Resume.exe entry, and updated EN/ES/FR/DE/TR Resume.exe localization copy with matching paragraph structure so language switching remains consistent. The rendered English career-break detail now opens with `Taking time to travel, recharge and enjoy the freedom to choose what comes next.` and describes travelling, enjoying life, personal projects, and freedom to decide the next chapter. Verification passed with red/green focused `node --test src/scripts/resume-app.test.mjs` and `node --test src/scripts/i18n.test.mjs`, full `node --test src/scripts/*.test.mjs` at 149/149, and `env ASTRO_TELEMETRY_DISABLED=1 npm run build` with 6 pages.
- 2026-07-03: Resume.exe timeline content refresh completed from the provided resume PDF plus user correction that Burak is no longer with Discover/Capital One and is currently on a career break. Updated `src/content/timeline.json` so Barclaycard is `Barclaycard (Barclays), Jun 2018 - Mar 2024`, Discover is `Discover Financial Services / Capital One, Mar 2024 - Apr 2026` with completed Agile & Technical Programme Manager and Scrum Master sections, and a new current `Career Break, Apr 2026 - Present` entry sits before `The Next Chapter`. Updated EN/ES/FR/DE/TR Resume.exe localization entries so language switching does not restore stale `Present` Discover copy. Added regressions for the current career-break timeline state and localized Discover/Career Break copy. Verification passed with red/green focused `node --test src/scripts/resume-app.test.mjs` and `node --test src/scripts/i18n.test.mjs`, full `node --test src/scripts/*.test.mjs` at 149/149, `env ASTRO_TELEMETRY_DISABLED=1 npm run build` with 6 pages, and built-output checks for `Career Break`, `Apr 2026 - Present`, and `Discover Financial Services / Capital One`.
- 2026-07-03: User-requested resume PDF repair completed. Root cause in repo history was commit `a69c33b` storing `public/Resume-BURAK-YUKSEL.pdf` as the empty Git blob even though the download UI only linked the asset; no source script was found that rewrites the PDF. Replaced the site asset with the provided `/Users/bd0t/Downloads/Resume-BURAK-YUKSEL.pdf` copy (54,852 bytes, PDF 1.7, 2 pages) and added a Downloads regression guard that fails unless the protected PDF is non-empty and starts with `%PDF-`. Verification passed with a red/green focused `node --test src/scripts/os-window-manager.test.mjs`, full `node --test src/scripts/*.test.mjs` at 148/148, `env ASTRO_TELEMETRY_DISABLED=1 npm run build` with 6 pages, and `dist/Resume-BURAK-YUKSEL.pdf` matching the public PDF byte-for-byte.
